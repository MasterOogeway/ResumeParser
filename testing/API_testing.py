...
if __name__ == "__main__":
    ...
    it_keywords = ["Software Developer", "IT Support", "Data Analyst", "Fresher", "Developer", "Python", "Java"]
    ...
    # 4. Test Adzuna API
    print("\nTesting Adzuna API...")
    # Values for ADZUNA_APP_ID and ADZUNA_APP_KEY will be loaded from .env
    if ADZUNA_APP_ID and ADZUNA_APP_KEY:
        print(f"Adzuna App ID: {'Found'}")
        print(f"Adzuna App Key: {'Found'}")
        # Adzuna uses country codes (e.g., 'gb', 'us', 'in'). We'll use 'in' for India.
        adzuna_jobs = fetch_adzuna_jobs(
            keywords=it_keywords,
            limit=test_limit,
            location_query=test_location, # The fetch_adzuna_jobs function in job_scraper should handle this
            country_code="in" # Example for India
        )
        print_job_summary("Adzuna API", adzuna_jobs)
    else:
        print("Skipping Adzuna API test: ADZUNA_APP_ID or ADZUNA_APP_KEY not found in environment.")
        print_job_summary("Adzuna API", [])

    # 5. Test JSearch API (RapidAPI)
    ...
    # 6. Test GitHub Jobs Mirror (uses Arbeitnow as proxy in your current scraper)
