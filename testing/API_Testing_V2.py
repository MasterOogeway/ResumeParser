...
try:
    from job_scraper_api_v1 import (
        ...
        fetch_jsearch_jobs,
        fetch_adzuna_jobs, # Added Adzuna fetch function
        USAJOBS_API_KEY, USAJOBS_USER_AGENT, # To check if keys are loaded
        RAPIDAPI_JSEARCH_KEY, # To check if keys are loaded
        ADZUNA_APP_ID, ADZUNA_APP_KEY # Added Adzuna keys for checking
    )
    ...
    print("Make sure all necessary functions (like fetch_adzuna_jobs) and key variables (ADZUNA_APP_ID, ADZUNA_APP_KEY) are defined and imported from it.")
    exit()
...
if __name__ == "__main__":
    ...
    # 1. Test Remotive API (Exactly as in user-provided file)
    print("Testing Remotive API (no key required)...")
    remotive_jobs = fetch_remotive_jobs(keywords=it_keywords + ["remote"], limit=test_limit)
    print_job_summary("Remotive API", remotive_jobs)

    # 2. Test Arbeitnow API (Exactly as in user-provided file)
    ...
    # 3. Test USAJOBS API (Exactly as in user-provided file)
    print("\nTesting USAJOBS API...")
    ...
    # 4. Test Adzuna API - MODIFIED FOR 5 SPECIFIC COUNTRIES
    print("\nTesting Adzuna API for specific countries (India, UK, US, Germany, Singapore)...")
    if ADZUNA_APP_ID and ADZUNA_APP_KEY:
        print(f"Adzuna App ID: {'Found'}")
        print(f"Adzuna App Key: {'Found'}")

        # Define the 5 countries you want to test
        adzuna_selected_countries = {
            "India": "in",
            "United Kingdom": "gb",
            "United States": "us",
            "Germany": "de",
        ...
        # Keywords and limit remain the same for this test iteration
        # it_keywords = ["Software Developer", "IT Support", "Data Analyst", "Fresher"] # Defined earlier
        # test_limit = 2 # Defined earlier

        for country_name, country_code in adzuna_selected_countries.items():
            print(f"\n-- Testing Adzuna for {country_name} ({country_code}) --")
            # For a broad search within each country, we might want to pass None or an empty string 
            # for location_query, rather than the fixed 'test_location' ("India"),
            # or your fetch_adzuna_jobs function should intelligently handle it.
            # Here, we'll pass None for location_query to search broadly within the country.
            adzuna_jobs = fetch_adzuna_jobs(
                keywords=it_keywords,
                limit=test_limit,
                location_query=None, # Search broadly within the country
                country_code=country_code
            )
            # Pass the country name to the summary for clarity
            print_job_summary(f"Adzuna API ({country_name})", adzuna_jobs)
    else:
        print("Skipping Adzuna API test: ADZUNA_APP_ID or ADZUNA_APP_KEY not found in environment.")
        print_job_summary("Adzuna API", [])

    # 5. Test JSearch API (RapidAPI) (Exactly as in user-provided file)
    print("\nTesting JSearch API (RapidAPI)...")
    # Value for RAPIDAPI_JSEARCH_KEY will be loaded from .env
    if RAPIDAPI_JSEARCH_KEY:
        print(f"RapidAPI JSearch Key: {'Found'}")
        jsearch_jobs = fetch_jsearch_jobs(keywords=it_keywords, limit=test_limit, location_query=test_location)
        print_job_summary("JSearch API (RapidAPI)", jsearch_jobs)
    else:
        print("Skipping JSearch API test: RAPIDAPI_JSEARCH_KEY not found in environment.")
        print_job_summary("JSearch API (RapidAPI)", [])

    # 6. Test GitHub Jobs Mirror (uses Arbeitnow as proxy in your current scraper) (Exactly as in user-provided file)
